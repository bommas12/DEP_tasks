async function pag(event) {
    ...
    if (event.target.innerText == "right" && state.curPage < state.totalPages) {
    ...
    } else if (event.target.innerText == "right" && state.curPage == state.totalPages) {
        state.curPage++;
        ...
        thumbnailSlider.appendChild(addVideoElements(thumbnailSlider.clientWidth, results, breakPoints.active.items));

        const ele = pagContainer.lastChild;
        pagContainer.removeChild(ele);
        pagContainer.appendChild(addPagination(results, breakPoints.active.items, state.totalPages));
        pagContainer.appendChild(ele);
    ...
    pagContainer.children[state.curPage].className = "active";
    ...
    slider.style.transform = 'translateX(' + -((state.curPage - 1) * breakPoints.active.items * thumbnailWidth) + 'px)';
